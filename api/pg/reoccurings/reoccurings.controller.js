const model = require('./reoccurings.model');
    ...
    if(id) {
        return model.getAll({id}).then(result => {
            if (result && result.rows && result.rows[0]) {
                return model.update(id, req.body).then(result => {
...
const deleteByWP = async (req, res, next) => {
    const workplan_id = req.params.id;
    if (workplan_id) {
        try {
            const response = await model.getAll({workplan_id});
            if (response.rows.length === 0) {
                return res.status(202).json({error: 'no reoccurings'});
            } else {
